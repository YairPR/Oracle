...
#------------------------------------------------------------------------------
grep -v "^#" ${_RunAwrListFile} | awk -F: '{print $1" "$2" "$3" "$4}' | \
while read _ListDb _ListRcpts _Dayinpast _ListHrs
do
 #----------------------------------------------------------------------
...
where   end_interval_time >= (sysdate-(${_day}))
...
from    dba_hist_snapshot
where   end_interval_time <= (sysdate-(${_day}))+(${_Hrs}/24)
and     dbid = &&V_DBID
and     instance_number = &&V_INST;
