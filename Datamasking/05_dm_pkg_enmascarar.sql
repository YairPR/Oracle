    ...
    SELECT COUNT(*), COUNT(DISTINCT table_name)
      ...
      FROM (
        WITH has_final AS (
          SELECT CASE
                   WHEN EXISTS (
                     SELECT 1
                       FROM tdm_columna_final f0
                      WHERE f_norm(f0.owner_name) = l_esquema
                        AND f_norm(f0.enmascarar) = 'Y'
                   ) THEN 1 ELSE 0
                 END v
            FROM dual
        )
        SELECT f.owner_name, f.table_name, f.column_name, f.identificador
          ...
          JOIN has_final hf ON hf.v = 1
         WHERE f_norm(f.owner_name) = l_esquema
           AND f_norm(f.enmascarar) = 'Y'
        UNION ALL
        SELECT f_norm(e.owner_name), f_norm(e.table_name), f_norm(e.column_name), e.identificador_forz
          FROM tdm_excepcion_col e
          JOIN has_final hf ON hf.v = 0
     ...
     WHERE 1=1
       ...
       AND NOT EXISTS (
         SELECT 1
           FROM tdm_excepcion_col e
          WHERE f_norm(e.owner_name) = src.owner_name
            AND f_norm(e.table_name) = src.table_name
            AND f_norm(e.column_name) = src.column_name
            AND f_norm(e.activa) = 'Y'
        ...
        FROM (
          ...
          SELECT f.owner_name, f.table_name, f.column_name
           ...
           WHERE f_norm(f.owner_name) = l_esquema
             AND f_norm(f.enmascarar) = 'Y'
          UNION ALL
          SELECT f_norm(e.owner_name), f_norm(e.table_name), f_norm(e.column_name)
            FROM tdm_excepcion_col e
            JOIN has_final hf ON hf.v = 0
           WHERE f_norm(e.owner_name) = l_esquema
             AND f_norm(e.activa) = 'Y'
      ...
      HAVING COUNT(*) = SUM(
               CASE
                 WHEN EXISTS (
                   SELECT 1
                     FROM tdm_excepcion_col e
                    WHERE f_norm(e.owner_name) = c.owner_name
                      AND f_norm(e.table_name) = c.table_name
                      AND f_norm(e.column_name) = c.column_name
                      AND f_norm(e.activa) = 'Y'
                      AND f_norm(e.accion) = 'EXCLUDE'
                 ) THEN 1 ELSE 0
               END
