...
column v_ruta        new_value V_RUTA        noprint
column v_esquema     new_value V_ESQUEMA     noprint
column v_dbname      new_value V_DBNAME      noprint
column v_fecha       new_value V_FECHA       noprint
column v_file        new_value V_FILE        noprint

select replace('&1','\','/') v_ruta from dual;
select upper(trim('&2')) v_esquema from dual;
