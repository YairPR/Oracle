...
-- 1. Descartar nombres de objetos técnicos, procesos y metadatos en IDENTIFICADOR_PERSONAL
INSERT INTO tdm_regla (regla_id, identificador, tipo_regla, expresion, puntuacion, prioridad, descripcion)
SELECT seq_dm_regla.nextval, 'IDENTIFICADOR_PERSONAL', 'COLUMN_NAME', 
       '(^|_)(NOMBRE)_(TABLA|COLUMNA|CAMPO|PROCESO|TAREA|JOB|CLASE|METODO|ROL|PERFIL|GRUPO|ESTADO|CATEGORIA|TIPO|PLANTILLA|REPORTE|INFORME|LOG|IMAGEN|RUTA|PATH|ACCION|VALOR|UNIDAD|MEDIDA)($|_)', 
       -320, 1, 'Descarte de nombres de metadatos/procesos técnicos'
FROM dual 
WHERE NOT EXISTS (
    SELECT 1 FROM tdm_regla 
    WHERE identificador = 'IDENTIFICADOR_PERSONAL' 
      AND tipo_regla = 'COLUMN_NAME' 
      AND expresion = '(^|_)(NOMBRE)_(TABLA|COLUMNA|CAMPO|PROCESO|TAREA|JOB|CLASE|METODO|ROL|PERFIL|GRUPO|ESTADO|CATEGORIA|TIPO|PLANTILLA|REPORTE|INFORME|LOG|IMAGEN|RUTA|PATH|ACCION|VALOR|UNIDAD|MEDIDA)($|_)'
);

-- 2. Descartar canales de pago/comunicación en IDENTIFICADOR_DIRECCION
INSERT INTO tdm_regla (regla_id, identificador, tipo_regla, expresion, puntuacion, prioridad, descripcion)
SELECT seq_dm_regla.nextval, 'IDENTIFICADOR_DIRECCION', 'COLUMN_NAME', 
       '(^|_)(VIA)_(PAGO|COBRO|COMUNICACION|ACCESO|TRANSMISION|RECLAMACION|CONTACTO)($|_)', 
       -260, 1, 'Descarte de canales técnicos (no son vías de domicilio)'
FROM dual 
WHERE NOT EXISTS (
    SELECT 1 FROM tdm_regla 
    WHERE identificador = 'IDENTIFICADOR_DIRECCION' 
      AND tipo_regla = 'COLUMN_NAME' 
      AND expresion = '(^|_)(VIA)_(PAGO|COBRO|COMUNICACION|ACCESO|TRANSMISION|RECLAMACION|CONTACTO)($|_)'
);

-- 3. Descartar tipos/estados de cuenta en IDENTIFICADOR_BANCARIO
INSERT INTO tdm_regla (regla_id, identificador, tipo_regla, expresion, puntuacion, prioridad, descripcion)
SELECT seq_dm_regla.nextval, 'IDENTIFICADOR_BANCARIO', 'COLUMN_NAME', 
       '(^|_)(TIPO|ESTADO|CLASE|GRUPO|CATEGORIA|MONEDA|DIVISA)_(CUENTA|IBAN|CCC)($|_)|(^|_)(CUENTA|IBAN|CCC)_(TIPO|ESTADO|CLASE|GRUPO|CATEGORIA|MONEDA|DIVISA)($|_)', 
       -320, 1, 'Descarte de tipos o metadatos de cuenta bancaria'
FROM dual 
WHERE NOT EXISTS (
    SELECT 1 FROM tdm_regla 
    WHERE identificador = 'IDENTIFICADOR_BANCARIO' 
      AND tipo_regla = 'COLUMN_NAME' 
      AND expresion = '(^|_)(TIPO|ESTADO|CLASE|GRUPO|CATEGORIA|MONEDA|DIVISA)_(CUENTA|IBAN|CCC)($|_)|(^|_)(CUENTA|IBAN|CCC)_(TIPO|ESTADO|CLASE|GRUPO|CATEGORIA|MONEDA|DIVISA)($|_)'
);

-- 4. Descartar tipos/banderas de verificación de teléfono en IDENTIFICADOR_TELEFONO
INSERT INTO tdm_regla (regla_id, identificador, tipo_regla, expresion, puntuacion, prioridad, descripcion)
SELECT seq_dm_regla.nextval, 'IDENTIFICADOR_TELEFONO', 'COLUMN_NAME', 
       '(^|_)(TIPO|COD|ID|FLAG|CHECK|SW|IND|VALIDO)_(TEL|TELEFONO|TFNO|MOVIL)($|_)|(^|_)(TEL|TELEFONO|TFNO|MOVIL)_(TIPO|COD|ID|FLAG|CHECK|SW|IND|VALIDO)($|_)', 
       -300, 1, 'Descarte de metadatos o flags de teléfono'
FROM dual 
WHERE NOT EXISTS (
    SELECT 1 FROM tdm_regla 
    WHERE identificador = 'IDENTIFICADOR_TELEFONO' 
      AND tipo_regla = 'COLUMN_NAME' 
      AND expresion = '(^|_)(TIPO|COD|ID|FLAG|CHECK|SW|IND|VALIDO)_(TEL|TELEFONO|TFNO|MOVIL)($|_)|(^|_)(TEL|TELEFONO|TFNO|MOVIL)_(TIPO|COD|ID|FLAG|CHECK|SW|IND|VALIDO)($|_)'
);
