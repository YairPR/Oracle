...
create or replace PACKAGE BODY pkg_dm_func_mask AS
  ...
  BEGIN
    IF g_pepper IS NULL THEN
      SELECT valor INTO g_pepper FROM tdm_secreto WHERE clave = 'PEPPER_MASK';
    END IF;
    RETURN g_pepper;
  EXCEPTION
  ...
  BEGIN
    RETURN ABS(DBMS_UTILITY.GET_HASH_VALUE(f_get_pepper||'|'||NVL(p_txt,'~NULL~'), 1, 2147483646));
  END;
  ...
  BEGIN
    ...
    IF REGEXP_LIKE(l_val,'^[0-9]{8}[A-Z]$') THEN
      l_orig_num := TO_NUMBER(SUBSTR(l_val, 1, 8));
      l_new_num := f_map_bijective(l_orig_num, 100000000);
      l_num8 := LPAD(TO_CHAR(l_new_num), 8, '0');
      l_out  := l_num8 || f_letra_dni(l_new_num);

    ELSIF REGEXP_LIKE(l_val,'^[XYZ][0-9]{7}[A-Z]$') THEN
      l_tipo := SUBSTR(l_val, 1, 1);
      l_orig_num := TO_NUMBER(SUBSTR(l_val, 2, 7));
      l_new_num := f_map_bijective(l_orig_num, 10000000);
      l_num7 := LPAD(TO_CHAR(l_new_num), 7, '0');
      l_out  := l_tipo || l_num7 ||
    ...
    ELSE
      ...
      ELSE
        ...
        l_new_num := MOD(l_seed*43+29, 10000000);
        l_num7 := LPAD(TO_CHAR(l_new_num), 7, '0');
        l_out  := l_tipo || l_num7 || f_cif_ctrl(l_tipo, l_num7);
      END IF;
    END IF;
  ...
  BEGIN
    ...
    BEGIN
      l_orig_num := TO_NUMBER(REGEXP_REPLACE(SUBSTR(p_valor,1,200),'[^0-9]',''));
      l_mod := POWER(10, l_target_len);
      l_new_num := f_map_bijective(l_orig_num, l_mod);
      l_out := LPAD(TO_CHAR(l_new_num), l_target_len, '0');
    EXCEPTION
      WHEN OTHERS THEN
  ...
  BEGIN
    ...
    IF REGEXP_LIKE(l_val, '^ES[0-9]{22}$') THEN
      l_orig_bban := TO_NUMBER(SUBSTR(l_val, 5, 20));
      l_new_bban := f_map_bijective(l_orig_bban, 100000000000000000000);
      l_bban := LPAD(TO_CHAR(l_new_bban), 20, '0');
      l_cc   := f_iban_cc_es(l_bban);
      l_out  := 'ES'||l_cc||l_bban;
    ELSE
      l_seed := f_hash('IBAN|'||UPPER(TRIM(SUBSTR(p_valor,1,50))));
      l_bnum := MOD(l_seed*137+31, 100000000000000000000);
      l_bban := LPAD(TO_CHAR(l_bnum), 20, '0');
      l_cc   := f_iban_cc_es(l_bban);
      l_out  := 'ES'||l_cc||l_bban;
  ...
  BEGIN
    ...
    FOR i IN 2 .. LENGTH(l_val)-1 LOOP
      ...
      IF REGEXP_LIKE(l_ch,'[0-9]') THEN
        ...
        l_mid  := l_mid || TO_CHAR(MOD(l_seed,10));
      ELSIF REGEXP_LIKE(l_ch,'[[:alpha:]]') THEN
        l_seed := MOD(l_seed*131+17,4294967291);
        l_mid  := l_mid || CHR(65 + MOD(l_seed,26));
      ELSE
        l_mid := l_mid || l_ch;
      END IF;
    END LOOP;
