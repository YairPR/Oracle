...
create or replace PACKAGE BODY pkg_dm_enmascarar AS
  ...
  ------------------------------------------------------------------------------
  -- FASE 1: Constructor de Mapas Biyectivos
  ------------------------------------------------------------------------------
  PROCEDURE proc_dm_build_domain_map(
  ...
  BEGIN
    LOOP
      FETCH p_valores INTO l_orig;
      EXIT WHEN p_valores%NOTFOUND;

      l_orig := UPPER(TRIM(l_orig));
      IF l_orig IS NULL THEN
        CONTINUE;
      END IF;

      -- Verificar si ya existe mapeo para este valor original en el dominio (idempotencia)
      SELECT COUNT(*) INTO l_exists
        FROM tdm_mask_key_map
       WHERE dominio = p_dominio
         AND valor_orig = l_orig;

      IF l_exists = 0 THEN
        l_n := 0;
        LOOP
          -- Generar valor enmascarado candidato
          l_masc := pkg_dm_func_mask.func_generico(
                      p_identificador,
                      CASE WHEN l_n = 0 THEN l_orig ELSE l_orig||'#'||l_n END
                    );

          BEGIN
            INSERT INTO tdm_mask_key_map(dominio, valor_orig, valor_masc)
            VALUES (p_dominio, l_orig, l_masc);
            EXIT; -- Insertado con éxito
          EXCEPTION
            WHEN DUP_VAL_ON_INDEX THEN
              -- Colisión con valor_masc de otro original, incrementamos sufijo y reintentamos
              l_n := l_n + 1;
              IF l_n > 100 THEN
                RAISE_APPLICATION_ERROR(-20201,
                  'No se pudo resolver la colision de clave para dominio: '||p_dominio||', original: '||l_orig);
              END IF;
          END;
        END LOOP;
      END IF;
  ...
  ------------------------------------------------------------------------------
  -- FASE 2: Pre-construcción de todos los mapas del esquema
  ------------------------------------------------------------------------------
  PROCEDURE proc_dm_pre_build_maps(
    p_esquema IN VARCHAR2
  ) IS
  BEGIN
    -- Bypass: Enmascaramiento determinista stateless sin almacenamiento de mapas
    NULL;
  END;
  ...
  BEGIN
    ...
    IF l_row_count > 100000 AND l_iot_type IS NULL THEN
      ...
      l_sql_chunk := p_sql_base || ' AND rowid BETWEEN :start_id AND :end_id';
  ...
  BEGIN
    ...
    -- FASE 2: Enmascarar utilizando la correspondencia biyectiva del mapa de dominio si existe
    BEGIN
      SELECT dominio INTO l_dominio
    ...
    IF FALSE AND l_dominio IS NOT NULL THEN
      DECLARE
        l_dom_clean VARCHAR2(256) := REPLACE(l_dominio, '''', '''''');
        l_no_map    NUMBER := 0;
      BEGIN
        -- 1) PRE-CHEQUEO de cobertura: originales que NO están en el mapa
        EXECUTE IMMEDIATE
          'SELECT COUNT(*) FROM '||f_qname(p_owner)||'.'||f_qname(p_tabla)||' d '||
          ' WHERE d.'||f_qname(p_columna)||' IS NOT NULL '||
          '   AND NOT EXISTS (SELECT 1 FROM tdm_mask_key_map m '||
          '                    WHERE m.dominio = :1 '||
          '                      AND m.valor_orig = UPPER(TRIM(d.'||f_qname(p_columna)||')))'
          INTO l_no_map
          USING l_dominio;
        ...
        IF l_no_map > 0 THEN
          proc_dm_trace(
            ...
            'MASK',
            'APPLY_COL_MAP_GAP',
            l_target||' valores sin mapear: '||l_no_map
          );
          RAISE_APPLICATION_ERROR(-20202, 'Cobertura de mapa incompleta ('||l_no_map||' sin mapear) en '||l_target);
        END IF;

        -- 2) Aplicar (cobertura ya garantizada; sin COALESCE porque no puede fallar)
        l_sql := 'UPDATE '||f_qname(p_owner)||'.'||f_qname(p_tabla)||' d '||
                 '   SET '||f_qname(p_columna)||' = (SELECT m.valor_masc FROM tdm_mask_key_map m '||
                 '                                     WHERE m.dominio = '''||l_dom_clean||''''||
                 '                                       AND m.valor_orig = UPPER(TRIM(d.'||f_qname(p_columna)||')))'||
                 ' WHERE '||f_qname(p_columna)||' IS NOT NULL';
        proc_dm_ejecuta_update_seguro(p_owner, p_tabla, p_columna, l_sql, l_rows);
        COMMIT;

        proc_dm_trace(p_solicitud_id, p_ejecucion_id, 'MASK', 'APPLY_COL_MAP', l_target||' (via mapa de dominio) filas='||l_rows);
        RETURN;
      END;
    END IF;
      ...
      IF SQLCODE = -1 THEN
        IF l_data_type IN ('CHAR','VARCHAR2','NCHAR','NVARCHAR2') AND NVL(l_char_len,0) > 0 THEN
          ...
          l_expr :=
            ...
            '||LPAD(TO_CHAR(MOD(ORA_HASH('||f_qname(p_columna)||'),1000000)),6,''0'')'||
  ...
  BEGIN
    ...
    -- Pre-construir mapas de dominio para asegurar consistencia e integridad referencial (biyección)
    BEGIN
      proc_dm_trace(l_solicitud_id, p_ejecucion_id, 'MASK', 'PRE_BUILD_MAPS_START', 'Inicio pre-construccion de mapas biyectivos.');
      proc_dm_pre_build_maps(l_esquema);
      proc_dm_trace(l_solicitud_id, p_ejecucion_id, 'MASK', 'PRE_BUILD_MAPS_END', 'Fin pre-construccion de mapas.');
    EXCEPTION
      WHEN OTHERS THEN
        proc_dm_trace(l_solicitud_id, p_ejecucion_id, 'MASK', 'PRE_BUILD_MAPS_ERR', 'Error general pre-construyendo mapas: '||SQLERRM);
    END;
