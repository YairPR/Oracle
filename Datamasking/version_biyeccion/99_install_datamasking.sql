...
declare
  l_cnt number;
begin
  select count(*) into l_cnt from tdm_secreto where clave = 'PEPPER_MASK';
  if l_cnt = 0 then
    insert into tdm_secreto (clave, valor)
    values ('PEPPER_MASK', DBMS_RANDOM.STRING('X', 40));
    commit;
    dbms_output.put_line('Semilla PEPPER_MASK generada (aleatoria). NO se muestra por seguridad.');
  else
    dbms_output.put_line('Semilla PEPPER_MASK ya existe. Se conserva.');
  end if;
end;
/
